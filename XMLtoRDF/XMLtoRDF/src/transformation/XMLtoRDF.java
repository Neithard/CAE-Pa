...
public class XMLtoRDF {
	...
	public boolean transformation() {
				...
				doc = openXML(new File(source_file));
				
				//wenn Fehler beim Einlesen, dann Programm beenden
				if(doc==null)
					return false;
				...
				//Präfix setzen
				model.setNsPrefix( "CAE", namepsace );
										...
										for(int k = k_start; k < RevInfo.getLength(); k++) {
											...
											if(eRevision.getAttribute("Beschreibung").contains("Erstellt von")){
												...
												revision.addProperty(created, model.createResource().addLiteral(name, eRevision.getAttribute("Wert"))
																									.addLiteral(date, OleAutomationDateUtil.fromOADate(eRevElementNextElement.getAttribute("Wert"), tz).toString()));
...
//				String fileName = "RDF.rdf";
				FileWriter out;
		        try {
		        	out = new FileWriter(destination_file);
...
//		         }
	...
	private Document openXML(File fXmlFile){
		...
		try {
			...
			System.out.println("XML-Datei konnte nicht gefunden werden!");
			return null;
		}
